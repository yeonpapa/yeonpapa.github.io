...
- custom bridge 네트워크 : 사용자가 네트워크를 생성함. 같은 이름의 네트워크에 컨테이너가 생성되면 컨테이너 이름으로 통신이 가능 
```ping <container_name>``` 하면 ping이 먹는다. <br> ```docker network create --driver=bridge my-bridge```
